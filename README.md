...
- **Node.js 14.17.6** — included as a portable binary at `tools/node/`. Auto-configured via `.bashrc` when inside the `ag-local` folder. No system-level Node.js installation required.

The `.bashrc` configuration automatically sources `dev.sh` and adds `tools/node/` to `PATH` whenever you `cd` into the `ag-local` directory. All commands (`node`, `npm`, `ng`, and all dev functions) are immediately available.
...
Open **4 terminals** from the `ag-local` root (dev commands are auto-available):
...
All commands are defined in `dev.sh` and are available in any terminal inside `ag-local`.
...
```bash
# Place the export files in the ag-local root, then:
